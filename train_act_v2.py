...
Key optimizations applied:
  ...
  - CLIP language embedding cached once (frozen) — saves ~50 ms/batch
...
def train():
    ...
    # ---- CLIP language embedding (frozen, computed once) ------------------
    print("Loading CLIP …")
    tokenizer  = CLIPTokenizer.from_pretrained("openai/clip-vit-base-patch32")
    text_model = CLIPTextModel.from_pretrained("openai/clip-vit-base-patch32").to(device)
    text_model.eval()   # keep BN/dropout in eval — we never update CLIP
    ...
    # Free CLIP encoder after embedding is cached — reclaim ~300 MB VRAM
    ...
    for epoch in range(EPOCHS):
        ...
        for imgs, chunks in pbar:
            ...
            lang_emb = lang_emb_single.expand(imgs.size(0), -1)   # view, no copy
        ...
        with torch.no_grad():
            for imgs, chunks in tqdm(val_loader, desc=f"Epoch {epoch+1:02d}/{EPOCHS} [val] ", leave=False):
                ...
                chunks = chunks.to(device, non_blocking=True)
                lang_emb = lang_emb_single.expand(imgs.size(0), -1)
        ...
        # Save checkpoint every epoch (overwrites — keep best separately if needed)
